...
import hoodieMoss from "./assets/optimized/rtmn-hoodie-moss.webp";
...
const copy = {
  en: {
    ...
    heroKicker:"RTMN / 002 · GERMANY", heroTitle:"YOUR UNIFORM\nYOUR RULES.", heroText:"RAW TRUE MODERN NEW.", heroPrimary:"Shop new drop", heroSecondary:"", shipping:"Free shipping in Germany over €100", promo:"COMPLIMENTARY SHIPPING / GERMANY", viewAll:"All objects", featured:"Selected pieces", collection:"New drop", filters:"Refine", sort:"Order", filterAll:"All", newest:"Newest", priceLow:"Price: low to high", priceHigh:"Price: high to low", soldOut:"Sold out",
    ...
    valuesTitle:"Built different.", valuesText:"RTMN is a Berlin-inspired wardrobe for people who care about shape, texture and consistency.", value1:"Heavy materials", value1t:"Dense fabrics chosen to hold their shape.", value3:"Designed in Germany", value3t:"Minimal, functional and made for repeat wear.",
  ...
  de: {
    ...
    valuesTitle:"Anders gedacht.", valuesText:"RTMN ist eine Berlin-inspirierte Garderobe für Menschen mit Sinn für Form, Textur und Konsequenz.", value1:"Schwere Materialien", value1t:"Dichte Stoffe, die ihre Form behalten.", value3:"Designed in Deutschland", value3t:"Minimal, funktional und für den Alltag.",
  ...
  uk: {
    ...
    heroKicker:"RTMN / 002 · НІМЕЧЧИНА", heroTitle:"ТВІЙ УНІФОРМ\nТВОЇ ПРАВИЛА.", heroText:"RAW TRUE MODERN NEW.", heroPrimary:"Дивитися новий дроп", heroSecondary:"", shipping:"Безкоштовна доставка по Німеччині від 100 €", promo:"БЕЗКОШТОВНА ДОСТАВКА / НІМЕЧЧИНА", viewAll:"Усі об’єкти", featured:"Обрані речі", collection:"Новий дроп", filters:"Уточнити", sort:"Порядок", filterAll:"Усі", newest:"Новинки", priceLow:"Ціна: від дешевих", priceHigh:"Ціна: від дорогих", soldOut:"Немає в наявності",
    ...
    valuesTitle:"Зроблено інакше.", valuesText:"RTMN — гардероб у дусі Берліна для тих, кому важливі форма, текстура та послідовність.", value1:"Щільні матеріали", value1t:"Тканини, які тримають форму.", value3:"Дизайн у Німеччині", value3t:"Мінімально, функціонально, на щодень.",
...
function App(){
  ...
  return <div className="site-shell">
    <a className="skip-link" href="#main-content">{lang==="uk"?"До основного вмісту":lang==="de"?"Zum Inhalt":"Skip to content"}</a>
    ...
    {view==="shop" && <RTMNJourney>
      <RTMNHero lang={lang} onExplore={()=>scrollTo("catalog")} onScroll={scrollTo}/>

      <section className="catalog-section" id="catalog">
        <div className="section-head"><div><div className="eyebrow">RTMN / SHOP</div><h2>{t.collection}</h2></div><button className="text-button" onClick={()=>{setCollection("all");setCategory("All");setFilters(emptyFilters);setQuery("")}}>{t.viewAll}<Icon name="arrow"/></button></div>
        ...
        {filtered.length?<div className="product-grid">{filtered.map((p,i)=><ProductCard key={p.id} product={p} t={t} liked={wishlist.includes(p.id)} onLike={()=>toggleWishlist(p.id)} onOpen={()=>openProduct(p)} index={i}/>)}</div>:<div className="no-results"><div className="no-results-mark">RTMN</div><h3>{t.noResults}</h3><button className="button button-dark" onClick={()=>{setCollection("all");setCategory("All");setQuery("");setFilters(emptyFilters)}}>{t.clearFilters}</button></div>}
      </section>
